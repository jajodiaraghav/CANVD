...
$muts = $dbh->prepare($query);
$muts->execute();
while ($mut = $muts->fetch(PDO::FETCH_ASSOC)) {
	$mut_arr[$mut['Peptide_EnsPID']][] = $mut;
}
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
{
	...
	if(isset($mut_arr[$row['Peptide_EnsPID']])) {
		foreach ($mut_arr[$row['Peptide_EnsPID']] as $mut) {			
			...
			$col[23] = '-';
			$col[24] = '-';
			$col[25] = "Domain:".$row['Domain'].";Type:".$row['Type'].";DomainStartPos:".$row['DomainStartPos'].";DomainEndPos:".$row['DomainEndPos'].";DomainSequence:".$row['DomainSequence'].";Domain_InterPro_ID:".$row['Domain_InterPro_ID'].";PWM:".$row['PWM'];
			...
			$col[36] = "Mutation_ID:".$mut['ID'].";Mut_Description:".$mut['mut_description'].";Tumour_Site:".$mut['tumour_site'].";Mutation,Source_ID:".$mut['Mutation_Source_ID'].";Source:".$mut['Source'];
			$col[37] = '-';
			$col[38] = '-';
			$col[39] = '-';
			$col[40] = '-';
